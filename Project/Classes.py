...
class File:
    ...
    def is_last_modified(self):
        new_time = os.path.getmtime(self.path)
        if new_time != self.last_modified:
            self.is_changed = True
            self.last_modified = new_time  # Update last_modified when the file changes
...
class Commit:
    def __init__(self, message, list_files):
        self.commit_id = id(self)  # Unique commit ID based on object ID
...
class Repository:
    def __init__(self, name_repo: str):
        ...
        self.wit_ignore = [".venv",".venv1",".idea","__pycache__",".wit"]
...
if __name__ == "__main__":
    ...
    print_structure(repo.list_files)
