...
def init():
    ...
    try:
        if not os.path.exists(".wit"):
            Function.create_directory(f"{os.getcwd()}\\.wit", "Hello, the directory created successfully!")
...
def commit(message):
    ...
    try:
        ...
        Function.create_directory(id_commit_path, f"Directory id {id_commit_path} created successfully.")
